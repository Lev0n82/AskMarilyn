...
import { LayoutDashboard, LogOut, PanelLeft, Users, AlertTriangle, ClipboardList, FileText, Bot, ShieldCheck, Activity, Info, BookOpen, FlaskConical, PackageOpen, Settings2, FileSpreadsheet, Monitor, HelpCircle } from "lucide-react";
...
const ALL_MENU_ITEMS: Array<{ icon: React.ElementType; label: string; path: string; featureKey?: GraceFeatureKey }> = [
  // ── Main ──────────────────────────────────────────────────
...
function DashboardLayoutContent({
  ...
  const menuItems = ALL_MENU_ITEMS.filter(item => !item.featureKey || isEnabled(item.featureKey));
  ...
  return (
    <>
      ...
      <SidebarInset>
        ...
        <main id="main-content" className="flex-1 p-4" tabIndex={-1}>{children}</main>
      </SidebarInset>
    </>
  );
}
