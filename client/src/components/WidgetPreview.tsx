  ...
  return (
    <div className="relative w-full h-[420px] rounded-xl border border-slate-200 overflow-hidden">
      ...
      <div className={`absolute inset-0 ${
        theme === "Aurora Soft" ? "bg-slate-900" 
        : theme === "Liquid Glass" ? "bg-gradient-to-br from-indigo-50 via-purple-50/40 to-sky-50"
        : "bg-amber-50/30"
      }`}>
        <div className="p-4 space-y-2">
          <div className={`h-4 w-32 rounded ${
            theme === "Aurora Soft" ? "bg-slate-700" 
            : theme === "Liquid Glass" ? "bg-white/60 border border-white/40" 
            : "bg-stone-200"
          }`} />
          <div className={`h-3 w-full rounded ${
            theme === "Aurora Soft" ? "bg-slate-800" 
            : theme === "Liquid Glass" ? "bg-white/40" 
            : "bg-stone-100"
          }`} />
          ...
          <div className={`h-20 w-full rounded-lg ${
            theme === "Aurora Soft" ? "bg-slate-800" 
            : theme === "Liquid Glass" ? "bg-white/50 border border-white/30" 
...
function getPreviewThemeClasses(theme: string) {
  switch (theme) {
    case "Liquid Glass":
      return {
        pill: "bg-white/70 backdrop-blur-2xl border border-white/40 text-slate-700 shadow-[0_8px_32px_rgba(99,102,241,0.15)]",
        ...
        chip: "border-indigo-200/50 text-indigo-700 bg-indigo-50/40 backdrop-blur-sm",
        input: "border-white/40 bg-white/30 backdrop-blur-sm text-slate-400",
        userBubble: "bg-gradient-to-r from-indigo-500 to-purple-500 text-white",
        assistantBubble: "bg-white/60 backdrop-blur-sm text-slate-800 border border-white/40",
        sendBtn: "bg-gradient-to-r from-indigo-500 to-purple-500 text-white",
      };
    case "Aurora Soft":
      return {
    ...
    default:
      return {
        ...
        chip: "border-amber-200 text-stone-700 bg-amber-50",
        ...
        sendBtn: "bg-amber-600 text-white",
      };
  }
}
